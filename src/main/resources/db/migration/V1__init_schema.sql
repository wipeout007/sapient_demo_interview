...
CREATE INDEX idx_movie_title ON movie_details(title);
...
CREATE INDEX idx_theatre_city_id ON movie_theatre(city_id);


CREATE TABLE movie_shows (
    id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    theatre_id       UUID NOT NULL REFERENCES movie_theatre(id),
    movie_id         UUID NOT NULL REFERENCES movie_details(id),
...
CREATE INDEX idx_show_theatre_id        ON movie_shows(theatre_id);
CREATE INDEX idx_show_movie_id          ON movie_shows(movie_id);
CREATE INDEX idx_show_date              ON movie_shows(show_date);
CREATE INDEX idx_show_movie_city_date   ON movie_shows(movie_id, show_date);

CREATE TABLE movie_show_seat (
    id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    show_id      UUID NOT NULL REFERENCES movie_shows(id),
...
CREATE INDEX idx_movie_booking_show_id       ON movie_booking_details(show_id);

CREATE TABLE movie_booking_item (
    id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    booking_id    UUID NOT NULL REFERENCES movie_booking_details(id),
    show_seat_id  UUID NOT NULL REFERENCES movie_show_seat(id),
